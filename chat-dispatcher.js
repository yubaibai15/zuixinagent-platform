...
(() => {
  ...
  const rules = {
    visual: [{ test: /\u89c6\u9891|\u77ed\u89c6\u9891|\u811a\u672c|\u5206\u955c|\u955c\u5934|\u62cd\u6444|\u53e3\u64ad/i, label: '短视频分镜与素材表', href: '/downloads/visual-video-storyboard-with-images.xlsm', link: '下载 AI 短视频素材表 Excel', delay: 450, download: true }],
  ...
  const show = async (query, rule) => {
    ...
    const wait = document.getElementById('role-skill-wait'); if (wait) wait.outerHTML = `<div class="bubble bot">已完成「${esc(rule.label)}」。<br><a class="skill-result-link" href="${rule.href}"${rule.download ? ' download' : ''}>${esc(rule.link)} →</a></div>`;
