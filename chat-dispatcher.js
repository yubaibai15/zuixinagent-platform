...
(() => {
  ...
  const rules = {
    visual: [{ test: /\u89c6\u9891|\u77ed\u89c6\u9891|\u811a\u672c|\u5206\u955c|\u955c\u5934|\u62cd\u6444|\u53e3\u64ad/i, label: '圣灵节短视频分镜脚本', href: '/downloads/圣灵节专场营销-短视频分镜脚本.xlsx?v=20260923', filename: '圣灵节专场营销-短视频分镜脚本.xlsx', link: '下载已完成的短视频分镜脚本 Excel', delay: 3000, download: true }],
    marketing: [
      { test: /(?:\u91c7\u96c6|\u722c\u53d6|\u5bfc\u51fa|\u4e0b\u8f7d).{0,8}\u76f4\u64ad|\u76f4\u64ad.{0,8}(?:\u6570\u636e|excel|\u8868\u683c|\u91c7\u96c6|\u722c\u53d6|\u5bfc\u51fa|\u4e0b\u8f7d)/i, label: '直播数据 Excel', href: '/downloads/A企业、竞品直播数据.xlsx', filename: 'A企业、竞品直播数据.xlsx', link: '下载直播数据 Excel', delay: 3000, download: true },
      { test: /\u5168\u5e74.{0,4}(?:\u8425\u9500)?\u65e5\u5386|\u8425\u9500\u65e5\u5386|\u65e5\u5386.{0,6}\u8425\u9500/, label: '全年营销日历', href: '/results/marketing-calendar.html', link: '打开全年营销日历', delay: 3000 }
    ],
    data: [
      { test: /\u5ba2\u6237\u753b\u50cf|\u7528\u6237\u753b\u50cf|\u753b\u50cf.{0,6}\u770b\u677f/, label: '客户画像看板', href: '/results/customer-profile-dashboard.html', link: '全屏打开客户画像看板', delay: 3000 },
      { test: /\u6570\u636e\u770b\u677f|\u6570\u636e\u5206\u6790|\u7ecf\u8425\u6570\u636e|\u770b\u770b\u677f/, label: '数据分析看板', href: '/results/data-dashboards.html', link: '打开数据分析看板', delay: 3000 }
    ]
  };
  // 在用户点击入口前预加载固定分镜表，后续下载优先使用浏览器缓存。
  const storyboardUrl = '/downloads/圣灵节专场营销-短视频分镜脚本.xlsx?v=20260923';
  ...
  const show = async (query, rule) => {
    ...
    const steps = ['正在识别你的需求与关键词…','正在匹配对应岗位技能与资料…','正在校验成果内容与打开方式…','已完成校验，正在整理最终结果…'];
    const waitId = `role-skill-wait-${Date.now()}`;
    input.value = ''; messages.insertAdjacentHTML('beforeend', `<div class="bubble me">${esc(query)}</div><div class="bubble bot" id="${waitId}">思考中（1/4）${steps[0]}</div>`); messages.scrollTop = messages.scrollHeight;
    ...
    if (wait) wait.outerHTML = `<div class="bubble bot">已完成「${esc(rule.label)}」。<br><a class="skill-result-link" href="${href}"${rule.download && rule.filename ? ` download="${esc(rule.filename)}"` : (rule.download ? ' download' : '')}>${esc(rule.link)} →</a></div>`;
